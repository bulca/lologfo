...
<div class="page-header">
	<h1><?=$type->name?><small>Purchased Accounts</small></h1>
	...
	<div class="col-md-3">
		<select class="form-control" id="changeType">
			<?php foreach($types as $t): ?>
				<option value="<?=site_url('/my-stuff/' . $t->type)?>" <?=$type->type == $t->type ? 'selected="selected"' : ''?>><?=$t->name?></option>
			<?php endforeach; ?>
		</select>
	</div>
	<div class="col-md-3">
				...
				<table id="accounts" class="table table-striped table-hover" cellspacing="0" width="100%">
			        <thead>
			            <tr>
			            	...
			            	<th align="right" width="10px">ID</th>
					...
					<div class="col-xs-6">
						<div class="dataTables_paginate paging_bootstrap">
							<ul class="pagination">
								...
								<li class="prev <?=$page['back'] == $page['current'] ? 'disabled': ''?>"><a href="<?=site_url('/my-stuff/' . $type->type . '?' . http_build_query($_GET))?>">← Previous</a></li>
								<?php if($page['back'] != $page['current']): ?>
									<?php $_GET['page'] = $page['back']; ?>
									<li><a href="<?=site_url('/my-stuff/' . $type->type . '?' . http_build_query($_GET))?>"><?=$page['back']?></a></li>
								<?php endif; ?>
								<?php $_GET['page'] = $page['current']; ?>
								<li class="active"><a href="<?=site_url('/my-stuff/' . $type->type . '?' . http_build_query($_GET))?>"><?=$page['current']?></a></li>
								<?php if($page['next'] != $page['current']): ?>
									<?php $_GET['page'] = $page['next']; ?>
									<li><a href="<?=site_url('/my-stuff/' . $type->type . '?' . http_build_query($_GET))?>"><?=$page['next']?></a></li>
								<?php endif;
									$_GET['page'] = $page['next'];
									?>
								<li class="next <?=$page['next'] == $page['current'] ? 'disabled': ''?>"><a href="<?=site_url('/my-stuff/' . $type->type . '?' . http_build_query($_GET))?>">Next → </a></li>
							</ul>
						</div>
					</div>
				</div>
